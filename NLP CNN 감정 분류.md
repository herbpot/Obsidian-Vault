이번엔 조금 오래 걸렸다. 데이터 전처리와 메모리 부족 문제 때문에 이를 해결하는데 계속 고민하다가 결국 데이터의 크기를 줄이기로 했다
...
# 5. #추가 
기존에 con2D는 2차원 데이터를 요구한다고 생각했었는데 3차원을 요구하여 이부분을 깨닫는것까지가 조금 오래 걸렸다. 또한 이 과정에서 강제적으로 2차원 데이터를 3차원으로 늘린 부분에 대해서 다른 방법은 없었는지에 대해 더 고민해봐야 할 것 같다. 
마지막으로 실제 사용을 위해서는 입력 문장의 전처리 과정이 필요할 것이다. 특히 \<UNK\>토큰사용에 대해서 구상을 조금 해봐야한다.

다음번에는 RNN을 이용한 감정 분류를 해보자